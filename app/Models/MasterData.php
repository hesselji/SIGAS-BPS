...
final class MasterData
{
    ...
    {
        return Database::connection()->query('SELECT code,name,prefix FROM letter_sensitivities WHERE is_active=1 ORDER BY sort_order')->fetchAll();
    }
    ...
    /**
     * KKA groups are now selected from the official classification catalogue.
     * Budget is kept as document metadata; it does not silently remove KKA options.
     */
    public static function classificationGroups(string $archiveType): array
    {
        $stmt = Database::connection()->prepare(
            'SELECT code,name,archive_type
             FROM classification_groups
             WHERE archive_type=? AND is_active=1
    ...
    {
        $stmt = Database::connection()->prepare(
            'SELECT code,raw_code,name,description
    ...
    /** Compatibility helper for old /api/classifications callers. */
    public static function classifications(string $scope, ?string $parent = null): array
    {
        ...
        if ($parent === null) {
            return array_map(
                fn(array $r) => ['code'=>$r['code'],'name'=>$r['name']],
                self::classificationGroups($archiveType)
            );
        }
        return array_map(
            fn(array $r) => ['code'=>$r['code'],'name'=>$r['name'],'description'=>$r['description']],
            self::classificationItems($parent)
        );
    }
    ...
    public static function numberingRules(): array
    {
        return Database::connection()->query(
            'SELECT nr.*,
                    (SELECT COUNT(*) FROM letter_types lt WHERE lt.numbering_rule_id=nr.id) AS type_count
    ...
    {
        $stmt = Database::connection()->prepare(
            'UPDATE numbering_rules SET name=?,unit_code=?,pattern=?,is_active=? WHERE id=?'
        );
    ...
    {
        $stmt = Database::connection()->prepare(
            'UPDATE letter_types SET numbering_rule_id=?,is_active=? WHERE id=?'
        );
    ...
    public static function classificationCatalog(array $filters = []): array
    {
        ...
        if (!empty($filters['archive_type'])) {
            ...
            $params['archive']=$filters['archive_type'];
        }
        if (!empty($filters['group'])) {
            $where[]='cg.code=:group_code';
            $params['group_code']=$filters['group'];
        }
        if (!empty($filters['q'])) {
